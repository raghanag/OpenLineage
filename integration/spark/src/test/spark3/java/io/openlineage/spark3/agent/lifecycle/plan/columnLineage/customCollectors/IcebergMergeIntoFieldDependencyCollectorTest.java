...
import io.openlineage.spark3.agent.lifecycle.plan.columnLineage.FieldDependenciesCollector;
...
public class IcebergMergeIntoFieldDependencyCollectorTest {
  ...
  IcebergMergeIntoFieldDependencyCollector collector =
      new IcebergMergeIntoFieldDependencyCollector();
  ...
  public void testCollectMatched() {
    ...
    try (MockedStatic<FieldDependenciesCollector> mockStatic =
        mockStatic(FieldDependenciesCollector.class)) {
      List<Option<Seq<Expression>>> matched = Arrays.asList(Option.apply(seq1), Option.apply(seq2));
      collector.collect(outputs, matched, Collections.emptyList(), builder);

      mockStatic.verify(
          () -> FieldDependenciesCollector.traverseExpression(expr1, outputExprId1, builder),
          times(1));
      mockStatic.verify(
          () -> FieldDependenciesCollector.traverseExpression(expr2, outputExprId2, builder),
          times(1));
      mockStatic.verify(
          () -> FieldDependenciesCollector.traverseExpression(expr3, outputExprId1, builder),
          times(1));
      mockStatic.verify(
          () -> FieldDependenciesCollector.traverseExpression(expr4, outputExprId2, builder),
          times(1));
    }
  }
  ...
  public void testCollectNotMatched() {
    ...
    try (MockedStatic<FieldDependenciesCollector> mockStatic =
      ...
      collector.collect(outputs, Collections.emptyList(), notMatched, builder);
